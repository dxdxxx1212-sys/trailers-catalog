#!/usr/bin/env python3
# ============================================================================
# !!! НЕ ЗАПУСКАТЬ !!!  DEPRECATED / УСТАРЕЛ (2026-09)
# Этот скрипт пересобирает каталог из merged_source.json, который УСТАРЕЛ:
#   - в нём нет ~32 товаров, добавленных напрямую в index.html (лодочные и др.) — они будут ПОТЕРЯНЫ;
#   - он заново вносит баги данных (грузоп>масса, тонны как кг, неверные оси) и ПЕРЕНУМЕРОВЫВАЕТ id
#     (ломает ссылки на товары и подборки).
# Источник правды теперь — window.PRODUCTS внутри index.html. Данные там уже вычищены.
# Добавлять товары — напрямую в index.html (или через ассистента), НЕ через этот скрипт.
# ============================================================================
# -*- coding: utf-8 -*-
